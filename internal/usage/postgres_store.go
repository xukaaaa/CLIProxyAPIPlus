...
func (s *PostgresUsageStore) Record(ctx context.Context, record coreusage.Record) error {
	...
	persisted := s.normalisePersistedUsageRecord(ctx, record)
...
func (s *PostgresUsageStore) Snapshot(ctx context.Context) (StatisticsSnapshot, error) {
	...
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`
		SELECT requested_at, api_key, model, source, auth_index, machine_id, failed, latency_ms,
...
func (s *PostgresUsageStore) normalisePersistedUsageRecord(ctx context.Context, record coreusage.Record) persistedUsageRecord {
...
func (s *PostgresUsageStore) MachineID() string {
	if s == nil {
		return ""
	}
	return s.machineID
