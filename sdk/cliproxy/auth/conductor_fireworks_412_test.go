...
func TestManagerMarkResult_Fireworks412SuspendsAccount(t *testing.T) {
	ctx := context.Background()
	model := "kimi-account-code"
	...
	auth1 := &Auth{ID: "fw-1", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1"}}
	auth2 := &Auth{ID: "fw-2", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1"}}
	if _, err := m.Register(ctx, auth1); err != nil {
		t.Fatalf("register fw-1: %v", err)
	}
	if _, err := m.Register(ctx, auth2); err != nil {
	...
	m.MarkResult(ctx, Result{
		...
		Model:    model,
		Success:  false,
		Error:    &Error{HTTPStatus: http.StatusPreconditionFailed, Message: "Account suspended"},
	})

	available := reg.GetAvailableModels("openai")
	for _, modelMap := range available {
		if id, ok := modelMap["id"].(string); ok && id == model {
			t.Fatalf("model %q should be suspended for the whole account, but found in available models", model)
		}
	}
}

func TestManagerMarkResult_Fireworks412EmptyModelSuspendsAllModels(t *testing.T) {
	ctx := context.Background()
	model := "kimi-empty-model"
	...
	updated2, ok := m.GetByID("fw-2")
	if !ok || updated2 == nil {
		t.Fatalf("fw-2 not found after mark result")
	}
	if state := updated2.ModelStates[model]; state == nil || !state.Unavailable {
		t.Fatalf("fw-2 model state for %q should be suspended", model)
	}
}

func TestManagerMarkResult_Fireworks412SuspendsAccountEvenWithDisableCooling(t *testing.T) {
	ctx := context.Background()
	model := "kimi-no-cool"
	...
	auth1 := &Auth{ID: "fw-1", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1", "disable_cooling": true}}
	auth2 := &Auth{ID: "fw-2", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1"}}
	if _, err := m.Register(ctx, auth1); err != nil {
		t.Fatalf("register fw-1: %v", err)
	}
	...
	for _, modelMap := range available {
		if id, ok := modelMap["id"].(string); ok && id == model {
			t.Fatalf("model %q should be suspended for the whole account even with disable_cooling", model)
		}
	}
}

func TestManagerMarkResult_Fireworks412RefreshesSiblingScheduler(t *testing.T) {
	ctx := context.Background()
	model := "kimi-scheduler"

	m := NewManager(nil, &RoundRobinSelector{}, nil)
	m.RegisterExecutor(&fireworks412Executor{})

	reg := registry.GetGlobalRegistry()
	reg.RegisterClient("fw-1", "fireworks", []*registry.ModelInfo{{ID: model}})
	reg.RegisterClient("fw-2", "fireworks", []*registry.ModelInfo{{ID: model}})
	reg.RegisterClient("fw-3", "fireworks", []*registry.ModelInfo{{ID: model}})
	t.Cleanup(func() {
		reg.UnregisterClient("fw-1")
		reg.UnregisterClient("fw-2")
	...
	auth1 := &Auth{ID: "fw-1", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1"}}
	auth2 := &Auth{ID: "fw-2", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-1"}}
	auth3 := &Auth{ID: "fw-3", Provider: "fireworks", Status: StatusActive, Metadata: map[string]any{"account_id": "acc-2"}}
	if _, err := m.Register(ctx, auth1); err != nil {
		t.Fatalf("register fw-1: %v", err)
	}
	if _, err := m.Register(ctx, auth2); err != nil {
	...
	if _, err := m.Register(ctx, auth3); err != nil {
		t.Fatalf("register fw-3: %v", err)
	}
	...
	picked, _, err := m.pickNext(ctx, "fireworks", model, cliproxyexecutor.Options{}, nil)
	if err != nil {
		t.Fatalf("pickNext error: %v", err)
	}
	if picked == nil || picked.ID != "fw-3" {
		t.Fatalf("pickNext = %v, want fw-3", picked)
	}
}

func TestManagerMarkResult_Fireworks412MixedCaseProviderSuspendsAccount(t *testing.T) {
	ctx := context.Background()
	model := "kimi-mixed-case"
	...
	for _, modelMap := range available {
		if id, ok := modelMap["id"].(string); ok && id == model {
			t.Fatalf("model %q should be suspended for the whole account even with mixed-case provider", model)
		}
	}
}
